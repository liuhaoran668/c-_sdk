...
struct TemperatureManager::Impl {
  ...
  std::array<int, 6> get_temperatures_blocking(double timeout_ms) {
    if (timeout_ms <= 0) {
  ...
  std::optional<std::pair<std::array<int, 6>, double>> get_current_temperatures() const {
    ...
    return std::make_pair(latest_data->temps, latest_data->timestamp);
  }
  ...
  void on_message(const CanMessage& msg) {
    ...
    for (std::size_t i = 0; i < kTemperatureCount; ++i) {
      data.temps[i] = static_cast<int>(msg.data[1 + i]);
    }
...
std::array<int, 6> TemperatureManager::get_temperatures_blocking(double timeout_ms) {
  return impl_->get_temperatures_blocking(timeout_ms);
}

std::optional<std::pair<std::array<int, 6>, double>> TemperatureManager::get_current_temperatures() const {
