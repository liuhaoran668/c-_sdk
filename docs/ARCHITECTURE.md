# LinkerHand C++ SDK 架构设计文档

> **文档版本**: 1.1
> **生成日期**: 2026-01-28
> **目标读者**: 架构师、高级开发者

---

## 1. 项目概览

### 1.1 项目定位与核心职责

LinkerHand C++ SDK 是一个用于控制 LinkerHand 机械手的底层通信库，通过 Linux SocketCAN 接口与硬件进行通信。项目提供：

- **CAN 总线通信抽象**: 封装 SocketCAN 操作，提供发布-订阅模式的消息分发
- **手型控制 API**: 支持 L6（六自由度灵巧手）和 O6（简化版）两种手型
- **传感器数据采集**: 角度、力传感器、扭矩、温度、电流、速度、故障信息

### 1.2 技术栈与依赖

| 组件 | 技术选型 |
|------|----------|
| 语言标准 | C++17 |
| 构建系统 | CMake 3.16+ |
| 线程库 | POSIX Threads (pthread) |
| CAN 通信 | Linux SocketCAN |
| 平台支持 | 仅 Linux |

### 1.3 目录结构与模块划分

```
jiagou/
├── include/linkerhand/
│   ├── linkerhand.hpp          # 统一入口头文件
│   ├── can_dispatcher.hpp      # CAN 消息调度器
│   ├── exceptions.hpp          # 异常类型定义
│   ├── iterable_queue.hpp      # 可迭代阻塞队列
│   └── hand/
│       ├── l6/                 # L6 手型模块
│       │   ├── l6.hpp
│       │   ├── angle_manager.hpp
│       │   ├── force_sensor_manager.hpp
│       │   ├── torque_manager.hpp
│       │   ├── speed_manager.hpp
│       │   ├── temperature_manager.hpp
│       │   ├── current_manager.hpp
│       │   └── fault_manager.hpp
│       └── o6/                 # O6 手型模块
│           ├── o6.hpp
│           ├── angle_manager.hpp
│           ├── torque_manager.hpp
│           ├── speed_manager.hpp
│           └── temperature_manager.hpp
└── src/
    ├── can_dispatcher.cpp
    └── hand/
        ├── common.hpp          # 内部共享工具
        ├── l6/*.cpp
        └── o6/*.cpp
```

---

## 2. 核心架构设计

### 2.1 整体架构图

```
┌─────────────────────────────────────────────────────────────────────┐
│                         Application Layer                           │
│                     (L6 / O6 Hand Interface)                        │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │                    L6 / O6 (Facade)                          │   │
│  │  ┌─────────┐ ┌─────────┐ ┌─────────┐ ┌─────────┐           │   │
│  │  │ Angle   │ │ Force   │ │ Torque  │ │ Temp    │  ...      │   │
│  │  │ Manager │ │ Sensor  │ │ Manager │ │ Manager │           │   │
│  │  └────┬────┘ └────┬────┘ └────┬────┘ └────┬────┘           │   │
│  │       │           │           │           │                 │   │
│  │       └───────────┴───────────┴───────────┘                 │   │
│  │                           │                                  │   │
│  └───────────────────────────┼──────────────────────────────────┘   │
│                              │ subscribe/send                       │
│  ┌───────────────────────────▼──────────────────────────────────┐   │
│  │                  CANMessageDispatcher                         │   │
│  │  ┌──────────────┐    ┌───────────────┐    ┌───────────────┐  │   │
│  │  │ recv_thread_ │───▶│ subscribers_  │───▶│  Callbacks    │  │   │
│  │  │ (poll loop)  │    │ (vector)      │    │  (dispatch)   │  │   │
│  │  └──────────────┘    └───────────────┘    └───────────────┘  │   │
│  │         ▲                                                     │   │
│  │         │ read/write                                          │   │
│  └─────────┼─────────────────────────────────────────────────────┘   │
│            │                                                         │
├────────────┼─────────────────────────────────────────────────────────┤
│            │                  System Layer                           │
│  ┌─────────▼─────────┐                                              │
│  │  Linux SocketCAN  │                                              │
│  │   (socket_fd_)    │                                              │
│  └───────────────────┘                                              │
└─────────────────────────────────────────────────────────────────────┘
```

### 2.2 核心类职责与协作关系

| 类名 | 职责 | 关键依赖 |
|------|------|----------|
| `L6` / `O6` | 外观类，聚合所有 Manager，管理整体生命周期 | `CANMessageDispatcher` |
| `CANMessageDispatcher` | CAN 消息收发与分发，管理接收线程 | Linux SocketCAN |
| `*Manager` | 特定传感器/执行器的数据管理，提供同步/异步/流式API | `CANMessageDispatcher` |
| `IterableQueue<T>` | 线程安全的阻塞队列，支持 range-based for 迭代 | - |

### 2.3 数据流向

```
[硬件] ──CAN Frame──▶ [SocketCAN] ──read()──▶ [recv_thread_]
                                                    │
                                                    ▼
                                            [on_message 回调]
                                                    │
                                         ┌──────────┴──────────┐
                                         ▼                      ▼
                                  [latest_data_]        [streaming_queue]
                                  (缓存最新值)           (流式数据队列)
                                         │                      │
                                         ▼                      ▼
                                [get_current_*()]      [stream() 迭代器]
```

---

## 3. 生命周期契约

### 3.1 资源管理模型

#### 3.1.1 `close()` / `stop()` / 析构函数 语义区分

| 方法 | 语义 | 幂等性 | 实现位置 |
|------|------|--------|----------|
| `L6::close()` / `O6::close()` | 停止所有流、关闭 dispatcher | ✅ 幂等 | `src/hand/l6/l6.cpp`, `src/hand/o6/o6.cpp` |
| `CANMessageDispatcher::stop()` | 停止接收线程、关闭 socket | ✅ 幂等 | `src/can_dispatcher.cpp` |
| `*Manager::stop_streaming()` | 停止流式数据采集 | ✅ 幂等 | `src/hand/**/**_manager.cpp` |
| 析构函数 | 调用 `close()`/`stop()`，吞掉异常 | N/A | 各类析构 |

#### 3.1.2 幂等性实现机制

**L6/O6 的 `close()` 实现**（示意）:
```cpp
void L6::close() {
  if (closed_.exchange(true, std::memory_order_acq_rel)) {  // 幂等 + 线程安全
    return;
  }
  // ... 停止各个流 ...
  dispatcher_.stop();
}
```

`closed_` 为 `std::atomic<bool>`，因此 `close()` 可安全并发调用，并且满足幂等性。

**CANMessageDispatcher 的 `stop()` 实现** (`can_dispatcher.cpp:117-130`):
```cpp
void CANMessageDispatcher::stop() {
  const bool was_running = running_.exchange(false);  // 原子交换，保证幂等
  if (was_running && recv_thread_.joinable()) {
    recv_thread_.join();
  }
  // ...
}
```

### 3.2 状态机定义

```
                    ┌──────────────────────────────────────────────┐
                    │              L6 / O6 状态图                   │
                    └──────────────────────────────────────────────┘

     ┌─────────┐         ┌───────────┐         ┌──────────┐
     │ Created │────────▶│  Running  │────────▶│  Closed  │
     └─────────┘         └───────────┘         └──────────┘
          │                    │                     │
          │ 构造函数           │ close()             │ 任何方法调用
          │                    │ 或析构              │
          ▼                    ▼                     ▼
     dispatcher_        stop_streaming()      ensure_open() 抛出
     初始化完成          + dispatcher_.stop()   StateError（或 dispatcher stop 后的状态错误）
```

### 3.3 异常后行为

#### 3.3.1 `close()` 后调用其他方法的统一行为

**当前实现行为**:

| 类 | close后调用行为 | 实现方式 |
|----|----------------|----------|
| `L6` / `O6` | `is_closed()==true`；`close()` 幂等；其成员 Manager 若触发 CAN IO 将抛 `StateError` | `closed_` 原子；dispatcher `send()` 状态检查 |
| `CANMessageDispatcher` | `send()` 在 `stop()` 后抛 `StateError("CAN dispatcher is stopped")`；`stop()` 幂等 | `running_` 原子 + `socket_mutex_` 双重检查 |
| `*Manager` | `get_current_*()` 仍可读取缓存；涉及 CAN IO 的操作会在 dispatcher stop 后抛 `StateError` | 依赖 dispatcher 的状态检查 + 各 Manager 内部状态机 |

**`ensure_open()` 实现**（示意）:
```cpp
void L6::ensure_open() const {
  if (closed_.load(std::memory_order_acquire)) {
    throw StateError(
        "L6 interface is closed. Create a new instance or use context manager.");
  }
}
```

**说明**: 当前 `L6/O6` 将各 Manager 作为公有成员直接暴露，因此 `ensure_open()` 不会自动覆盖到所有 Manager API；close 后的“状态约束”主要由 `CANMessageDispatcher::send()` 的状态检查与各 Manager 的内部状态机提供。

---

## 4. 并发模型与线程安全

### 4.1 线程模型

系统内部使用的线程：

| 线程 | 所属组件 | 职责 | 生命周期 |
|------|----------|------|----------|
| `recv_thread_` | `CANMessageDispatcher` | 轮询 CAN socket，分发消息 | 构造→stop() |
| `streaming_thread` | 各 `*Manager::Impl` | 周期性发送传感器请求 | stream()→stop_streaming() |
| `aggregation_thread` | `ForceSensorManager::Impl` | 聚合五指数据 | stream()→stop_streaming() |
| `reader_threads` | `ForceSensorManager::Impl` | 从单指队列读取数据 | 随 aggregation_thread |

### 4.2 回调执行上下文

**关键代码**（示意）:
```cpp
// 在 recv_thread_ 线程中执行
std::vector<std::shared_ptr<SubscriberState>> subscribers_copy;
{
  std::lock_guard<std::mutex> lock(subscribers_mutex_);
  subscribers_copy = subscribers_;  // 复制快照
}

for (const auto& subscriber : subscribers_copy) {
  if (!subscriber->active.load(std::memory_order_acquire)) {
    continue;  // 已取消订阅
  }
  try {
    subscriber->callback(msg);  // 回调在 recv_thread_ 中执行
  } catch (const std::exception& e) {
    std::cerr << "CANMessageDispatcher callback error: " << e.what() << "\n";
  }
}
```

#### 回调语义总结

| 属性 | 值 | 说明 |
|------|-----|------|
| **执行线程** | `recv_thread_` | 所有回调在同一线程顺序执行 |
| **是否允许阻塞** | ⚠️ 不建议 | 阻塞会延迟其他回调和后续消息处理 |
| **异常处理** | 捕获并记录到 stderr | 不会中断其他回调 |

### 4.3 订阅/取消订阅的并发策略

#### 4.3.1 `subscribe()` 实现（`src/can_dispatcher.cpp`）

```cpp
std::size_t CANMessageDispatcher::subscribe(Callback callback) {
  std::lock_guard<std::mutex> lock(subscribers_mutex_);
  const std::size_t id = next_subscription_id_++;
  subscribers_.push_back(std::make_shared<SubscriberState>(id, std::move(callback)));
  return id;
}
```

#### 4.3.2 `unsubscribe()` 实现（`src/can_dispatcher.cpp`）

```cpp
void CANMessageDispatcher::unsubscribe(std::size_t subscription_id) {
  std::lock_guard<std::mutex> lock(subscribers_mutex_);
  const auto it = std::find_if(...);
  if (it == subscribers_.end()) return;

  (*it)->deactivate();   // 标记为非活跃
  subscribers_.erase(it);
}
```

#### 并发安全性分析

| 操作 | 线程安全 | 可在回调中调用 | 说明 |
|------|----------|----------------|------|
| `subscribe()` | ✅ | ✅ | 使用 mutex 保护 |
| `unsubscribe()` | ✅ | ✅ | deactivate 后 recv_loop 跳过该订阅者 |

**快照模式的优点**: `recv_thread_` 在分发前复制 `subscribers_`，释放锁后再执行回调，允许回调中安全调用 `subscribe`/`unsubscribe`。

**架构约束**: 安全退订依赖于 `L6::close()` 的调用顺序——`dispatcher_.stop()` 会 join `recv_thread_`，确保 Manager 析构前没有回调在执行。

### 4.4 停止时的竞态处理

#### 4.4.1 `stop()` 与回调的协调

**当前实现分析** (`can_dispatcher.cpp:117-130`):

```cpp
void CANMessageDispatcher::stop() {
  const bool was_running = running_.exchange(false);  // 1. 设置停止标志
  if (was_running && recv_thread_.joinable()) {
    recv_thread_.join();  // 2. 等待线程结束
  }
  // 3. 关闭 socket
}
```

**`recv_loop()` 的退出机制**:
```cpp
while (running_.load()) {  // 检查停止标志
  const int ret = ::poll(&pfd, 1, 10);  // 10ms 超时
  // ...
}
```

**⚠️ 竞态窗口**: 在 `running_.exchange(false)` 后，`recv_thread_` 可能仍在执行回调（因为检查 `running_` 在循环开头）。`join()` 保证等待回调完成。

#### 4.4.2 Manager 的 `stop_streaming()` 实现

以 `AngleManager::Impl::stop_streaming()` 为例（示意）:

```cpp
void stop_streaming() {
  std::lock_guard<std::mutex> lock(streaming_mutex);
  if (!streaming_queue.has_value()) {
    return;  // 幂等保护
  }
  streaming_running.store(false);  // 1. 设置停止标志
  streaming_queue->close();        // 2. 关闭队列（唤醒阻塞消费者）
  streaming_queue.reset();         // 3. 清除内部引用
  if (streaming_thread.joinable()) {
    streaming_thread.join();       // 4. 等待线程结束
  }
}
```

**设计亮点**:
- `stop_streaming()` 幂等：允许多次调用；析构路径同样可安全复用
- `close()` 保证唤醒队列消费者，避免“析构时消费者永远阻塞”

**补充说明**:
- `stream()` 会先构造并返回队列对象（值语义、共享底层状态），避免并发 `stop_streaming()` 导致的返回引用失效问题。
- streaming 线程使用 `interval_ms` 的值拷贝，不依赖共享的 `optional` 状态，降低竞态与状态检查复杂度。

#### 4.4.3 析构与回调协调

当前实现采用**架构保证**而非复杂同步机制来确保安全析构：

1. `L6::close()` 调用 `dispatcher_.stop()`，join `recv_thread_`
2. join 返回后，`recv_loop` 不再执行，没有回调在运行
3. 之后 Manager 按声明逆序析构，`unsubscribe` 安全执行

**设计简化**：移除了 `in_flight` 计数和 `wait_for_idle()` 机制。安全性完全依赖于 `L6::close()` 保证的调用顺序。

---

## 5. 关键 API 使用指南

### 5.1 核心类的构造与初始化

```cpp
#include <linkerhand/linkerhand.hpp>

// 创建 L6 手型控制器
// 参数: side ("left"/"right"), interface_name ("can0"), interface_type ("socketcan")
linkerhand::L6 hand("left", "can0");

// 创建 O6 手型控制器
linkerhand::O6 hand_o6("right", "can1");
```

### 5.2 典型使用流程

#### 5.2.1 同步获取数据

```cpp
// 阻塞获取角度数据，超时 100ms
try {
  auto angles = hand.angle.get_angles_blocking(100);
  // angles.angles: std::array<int, 6>
  // angles.timestamp: double (Unix 时间戳)
} catch (const linkerhand::TimeoutError& e) {
  // 超时处理
}

// 获取缓存的最新值（非阻塞）
if (auto angles = hand.angle.get_current_angles()) {
  // 使用 angles->angles
}
```

#### 5.2.2 流式数据采集

```cpp
// 启动流，间隔 50ms，队列容量 100
auto queue = hand.angle.stream(50, 100);

// range-based for 迭代（阻塞）
for (const auto& data : queue) {
  // 处理 data
  if (should_stop) break;
}

// 停止流（或让 queue 离开作用域）
hand.angle.stop_streaming();
```

#### 5.2.3 设置角度

```cpp
// 使用 std::array
hand.angle.set_angles({128, 128, 128, 128, 128, 128});

// 使用 std::vector
std::vector<int> angles = {100, 100, 100, 100, 100, 100};
hand.angle.set_angles(angles);
```

### 5.3 错误处理约定

| 异常类型 | 触发场景 |
|----------|----------|
| `LinkerHandError` | 所有异常的基类 |
| `TimeoutError` | 阻塞操作超时 |
| `CANError` | CAN 通信错误 |
| `ValidationError` | 参数校验失败 |
| `StateError` | 状态不正确（如重复调用 stream()） |
| `QueueFull` / `QueueEmpty` | 队列操作异常 |
| `StopIteration` | 迭代器结束标记 |

---

## 6. 设计决策与权衡

### 6.1 设计取舍说明

| 决策 | 理由 | 权衡 |
|------|------|------|
| Pimpl 模式 | 隐藏实现细节，ABI 稳定 | 增加间接调用开销 |
| 快照分发 + 架构保证 | 允许回调中修改订阅；`stop()` join 保证析构安全 | 依赖调用顺序（`close()` 必须在 Manager 析构前） |
| 共享状态 Queue | `IterableQueue` 可复制，方便传递 | 需要 `shared_ptr`，增加引用计数开销 |
| 仅 Linux 支持 | 简化实现，专注 SocketCAN | 无法在非 Linux 平台使用 |

### 6.2 已知限制与潜在风险点

#### ✅ 已修复问题

| ID | 问题描述 | 修复摘要 |
|----|----------|----------|
| **F1** | `close()` 幂等但存在数据竞争风险 | `L6/O6::closed_` 使用 `std::atomic<bool>` + `exchange()` |
| **F2** | `unsubscribe()` 与回调并发导致析构期间 UAF | 通过架构保证：`stop()` join `recv_thread_` 后 Manager 才析构 |
| **F3** | dispatcher stop 后 send 行为未定义 | `CANMessageDispatcher::send()` 对 `running_` 与 `socket_fd_` 做状态检查并抛 `StateError` |
| **F4** | `stream()` / `stop_streaming()` 并发存在返回引用失效与竞态窗口 | `stream()` 返回队列值对象；streaming 线程使用 `interval_ms` 值拷贝；`stop_streaming()` 关闭队列并 join 线程 |
| **F5** | `ForceSensorManager` 聚合线程超时退出导致队列不关闭/潜在 UAF | 聚合循环对超时采用 continue；intermediate queue 使用共享所有权并在退出时关闭 |

#### 🟡 中等问题

| ID | 问题描述 | 影响 | 建议修复 |
|----|----------|------|----------|
| **M1** | `ensure_open()` 无法覆盖所有公开 API（Manager 作为公有成员暴露） | close 后的错误语义主要由 dispatcher/Manager 内部提供，Facade 层缺少统一入口与一致错误信息 | 让 Facade 提供封装方法或让 Manager 持有 Facade 的共享状态进行检查 |
| **M2** | 回调阻塞会影响整个消息分发 | 性能下降、消息堆积 | 文档说明；考虑异步分发 |
| **M3** | 对象生命周期与并发调用（尤其是阻塞 API） | 并发析构与正在执行的成员调用属于调用方责任 | 文档明确约束；必要时引入“取消阻塞等待”的机制 |

#### 🟢 轻微问题

| ID | 问题描述 | 影响 | 建议修复 |
|----|----------|------|----------|
| **L1** | `poll()` 超时 10ms 硬编码 | 不可配置 | 提取为构造参数 |
| **L2** | 错误信息输出到 stderr | 无法捕获或定制 | 引入日志回调 |

### 6.3 扩展性考量

#### 6.3.1 添加新的 Manager

1. 在 `include/linkerhand/hand/{version}/` 下创建头文件
2. 定义 `*Data` 结构体和 `*Manager` 类
3. 在 `src/hand/{version}/` 下实现 Impl
4. 在 `L6`/`O6` 类中添加成员并初始化

#### 6.3.2 支持新的 CAN 接口类型

当前仅支持 `socketcan`。扩展需要：

1. 修改 `CANMessageDispatcher` 构造函数的 `interface_type` 检查
2. 在 `#ifdef` 块中添加新的初始化逻辑
3. 考虑抽象出 `CANBackend` 接口

#### 6.3.3 跨平台支持

当前非 Linux 平台会：
- 构造时抛出 `CANError`
- `stop()` 直接返回（无操作）
- `send()` 直接抛出 `CANError`（SocketCAN 仅支持 Linux）

建议引入模拟后端或抽象接口以支持单元测试。

---

## 附录 A: 类图 (UML)

```
┌─────────────────────┐
│       L6/O6         │
├─────────────────────┤
│- dispatcher_        │
│- closed_            │
│+ angle: Manager     │
│+ torque: Manager    │
│+ ...                │
├─────────────────────┤
│+ close()            │
│+ is_closed()        │
└─────────┬───────────┘
          │ owns
          ▼
┌─────────────────────────────┐
│    CANMessageDispatcher     │
├─────────────────────────────┤
│- socket_fd_                 │
│- running_: atomic<bool>     │
│- recv_thread_               │
│- subscribers_: vector       │
├─────────────────────────────┤
│+ subscribe(cb) → id         │
│+ unsubscribe(id)            │
│+ send(msg)                  │
│+ stop()                     │
└─────────────────────────────┘
          ▲
          │ references
┌─────────┴───────────┐
│    *Manager         │
├─────────────────────┤
│- impl_: unique_ptr  │
├─────────────────────┤
│+ get_*_blocking()   │
│+ get_current_*()    │
│+ stream()           │
│+ stop_streaming()   │
└─────────────────────┘
```

---

## 附录 B: 消息协议摘要

| 功能 | CAN ID (left/right) | CMD Byte | 数据格式 |
|------|---------------------|----------|----------|
| 角度控制/查询 | 0x28 / 0x27 | 0x01 | [cmd, angle1..6] |
| 力传感器 (thumb) | 0x28 / 0x27 | 0xB1 | 多帧协议 (12 frames × 6 bytes) |
| 力传感器 (index) | 0x28 / 0x27 | 0xB2 | 同上 |
| ... | ... | ... | ... |

---

## 附录 C: 修改建议优先级

1. **短期改进 (M3)**: 明确并强化“对象生命周期与并发调用”的契约，必要时引入取消阻塞等待机制
2. **短期改进 (M1, M2)**: 提升 Facade 层一致性与回调侧约束说明
3. **长期优化 (L1, L2)**: 改善可维护性和可配置性

---

*文档结束*
