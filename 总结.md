...
`SubscriberState` 用于安全退订，防止回调执行中被销毁：
...
struct SubscriberState {
  ...
  bool try_enter();      // 检查 active，增加 in_flight
  void exit();           // 减少 in_flight，通知等待者
  void deactivate();     // 标记为非活跃
  void wait_for_idle();  // 等待正在执行的回调完成
};
```
...
void CANMessageDispatcher::unsubscribe(std::size_t subscription_id) {
  ...
  // 3. 避免死锁：recv_thread 中或自己的回调中退订，直接返回
